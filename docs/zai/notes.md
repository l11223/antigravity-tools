...
Goal: integrate z.ai as an upstream provider into NexusProxy’s proxy/service, primarily via an Anthropic API-compatible passthrough, and optionally provide “budget/usage” visibility and MCP helpers (search/reader/vision).
...
- z.ai is used **inside NexusProxy** (not via Google OAuth), but it must be able to serve **API Proxy** traffic alongside the existing account pool.
...
- z.ai MCP servers should be provided via NexusProxy’s proxy as optional toggles (enable/disable) and be usable by apps **without requiring users to configure z.ai keys** in the apps.
...
Practical implication for NexusProxy:
...
- For NexusProxy, the simplest first step is “treat `glm-*` as z.ai” (or require explicit `zai:` prefix), and forward model strings unchanged.
...
Behavior:
- Require NexusProxy’s local proxy auth (existing `api_key`) for access.
