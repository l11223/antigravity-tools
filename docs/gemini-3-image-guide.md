...
本文档详细说明了在 **NexusProxy** 项目中调用 Google `gemini-3-pro-image` (Imagen 3) 模型的方法。本项目已对该模型进行了 OpenAI 协议的完全兼容封装，并扩展支持了原生的摄影宽高比、人物生成安全策略，以及**图生图 (Image-to-Image)** 功能。
...
2.  **Q: 支持一次生成多张图片吗？**
    *   **A**: 支持。虽然 Gemini 上游单次请求限制生成 1 张，但 NexusProxy 代理层会自动并发处理 `n` 参数。例如设置 `n: 4`，系统会并行发起 4 个请求并合并结果返回。
